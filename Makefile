...
gradescope:
	...
	@if test "`git symbolic-ref HEAD`" = "refs/heads/kthread"; then \
		echo "ERROR: You are on the kthread branch, not your lab branch."; \
		false; \
	fi
	...
	rm -f submission.zip
	git archive --output=submission.zip HEAD $$(git diff -w --diff-filter=ACMRTUXB --name-only $$(git log --format="%H" -n 1 origin/kthread) HEAD)
